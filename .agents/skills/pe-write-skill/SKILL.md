---
name: pe-write-skill
description: Create or revise Pe.Tools agent skills after repeated workflow use proves a named routing surface is needed. Use when the user asks to add a skill, improve a skill, fix trigger reliability, fatten a skill, slash skill behavior, automatic skill activation, skill descriptions, modes or dispatch, resolver logic, scripts/assets in skills, or Pea-vs-repo skill boundaries.
metadata:
  goal: true
---
...
- Pea bundled skills and repo project skills must remain separate.

## Source of Truth

- Repo agent skills live under `.agents/skills/`.
- Pea bundled operator skills live separately in `source/pe-tools/packages/mcps/src/pea/skills.ts` and must not receive repo-development posture by accident.
...
Skill design decisions are usually durable. Capture changes to skill philosophy, source-of-truth rules, Pea-vs-repo skill separation, or trigger-routing policy in docs/context before or alongside skill source changes.
