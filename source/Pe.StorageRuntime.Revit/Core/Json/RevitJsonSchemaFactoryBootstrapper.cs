...
internal sealed class RevitJsonSchemaCapabilityAugmenter : IJsonSchemaCapabilityAugmenter {
    public void Configure(
        ...
        if (providerContext.AvailableCapabilityTier < SettingsCapabilityTier.RevitAssembly)
            return;
        ...
        settings.SchemaProcessors.Add(new RevitTypeSchemaProcessor(providerContext));
    }
}

internal static class RevitJsonSchemaModuleInitializer {
    [ModuleInitializer]
    internal static void RegisterAugmenter() => JsonSchemaFactory.RegisterAugmenter(new RevitJsonSchemaCapabilityAugmenter());
}
