...
public static class ElementContextCollector {
    public static ElementContextQueryData Collect(
        ...
        var resolution = ResolveQuery(doc, query, currentSelection, issues);
    ...
    private static QueryResolution ResolveQuery(
        ...
        return effectiveQuery.Kind switch {
            ...
            _ => ResolveCurrentSelection(doc, currentSelection)
        };
    }

    private static QueryResolution ResolveCurrentSelection(
        Document doc,
        IReadOnlyCollection<ElementId>? currentSelection
    ) {
