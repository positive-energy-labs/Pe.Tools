...
internal static class PeaLinkDevCommand {
    ...
    // The source-linked (dev-lane) pea/peco launcher. This lived in Pe.Shared.Product.PeaLauncherContent
    // as a second pea.cmd generator that also shipped baked into the installed payload; Phase 2 made the
    // SDK PathShim (ShimContent) the one installed-lane generator and moved the dev-only launcher here,
    // beside its sole consumer (`pe-dev pea link-dev`).
