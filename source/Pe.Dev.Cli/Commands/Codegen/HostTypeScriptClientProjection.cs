...
internal static class HostTypeScriptClientProjection {
    ...
    private static GeneratedProjectionFile[] GenerateFiles(
        ...
        return [
            ...
            new GeneratedProjectionFile(
                ...
                GenerateHostCapabilityMap(generatedHostTypeModel.ExportedTypeNames)
            )
        ];
    }
    ...
    private static string GenerateHostCapabilityMap(IReadOnlyDictionary<string, string> exportedTypeNames) => $$"""
        // <auto-generated />
        // Generated by `pe-dev codegen sync --target host-client` from HostOperationsCatalog.PublicHttp.
        ...
        export const hostCapabilityMap = {
          ...
          sections: [
        {{RenderHostCapabilityMapSections(exportedTypeNames, false)}}
          ],
          focusSections: [
        {{RenderHostCapabilityMapSections(exportedTypeNames, true)}}
          ],
        } as const satisfies HostCapabilityMap;
        """;

    private static string RenderHostCapabilityMapSections(
        ...
        return string.Join(
            ...
            sections
                ...
                .Select(section => RenderHostCapabilityMapSection(section, exportedTypeNames))
        );
    }
    ...
    private static string RenderHostCapabilityMapSection(
        HostCapabilitySectionProjection section,
        IReadOnlyDictionary<string, string> exportedTypeNames
    ) {
        ...
        foreach (var operation in section.Operations)
            AppendHostCapabilityMapRow(builder, operation, exportedTypeNames);
        _ = builder.AppendLine("      ],");
        _ = builder.Append("    }");
    ...
    private static void AppendHostCapabilityMapRow(
        ...
        var area = metadata.RevitLayer == null
            ? metadata.Domain
            : $"revit.{metadata.RevitLayer.ToString()!.ToLowerInvariant()}";
        ...
        _ = builder.AppendLine($"          area: {ToJsonString(area)},");
        _ = builder.AppendLine($"          description: {ToJsonString(metadata.Description)},");
        _ = builder.AppendLine($"          safety: {ToJsonString(safety)},");
        _ = builder.AppendLine($"          input: {ToJsonString(FormatShapeSummary(operation.RequestType, exportedTypeNames, true))},");
        _ = builder.AppendLine($"          output: {ToJsonString(FormatShapeSummary(operation.ResponseType, exportedTypeNames, false))},");
        _ = builder.AppendLine($"          relations: {ToJsonString(JoinPipe(metadata.RelatedOperations.Select(FormatRelatedOperation)))},");
        _ = builder.AppendLine($"          terms: {ToJsonString(JoinPipe(metadata.SearchTerms))},");
        _ = builder.AppendLine("        },");
    }

    private static string FormatShapeSummary(
        Type type,
        IReadOnlyDictionary<string, string> exportedTypeNames,
        bool request
    ) {
        if (request && type == typeof(NoRequest))
            return "omit";

        var typeName = GetOperationTypeName(type, exportedTypeNames, request);
        var fields = CreateShape(type, exportedTypeNames);
        if (fields.Count == 0)
            return typeName;
        ...
        var fieldSummary = string.Join(
            ...
            fields
                .Take(maxFields)
                .Select(field => $"{field.Name}{(field.Required ? "!" : string.Empty)}:{field.Type}")
        );
        var remaining = fields.Count - maxFields;
        return remaining > 0
            ? $"{typeName}({fieldSummary}, +{remaining})"
            : $"{typeName}({fieldSummary})";
    }

    private static string JoinPipe(IEnumerable<string> values) => string.Join("|", values.Where(value => !string.IsNullOrWhiteSpace(value)));

    private static string FormatRelatedOperation(HostOperationRelatedOperation operation) => string.IsNullOrWhiteSpace(operation.Note)
        ? $"{operation.Kind}:{operation.Key}"
        : $"{operation.Kind}:{operation.Key} ({operation.Note})";
