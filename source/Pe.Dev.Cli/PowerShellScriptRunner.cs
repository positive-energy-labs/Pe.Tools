...
internal static class PowerShellScriptRunner {
    public static async Task<int> RunForegroundAsync(
        ...
        var startInfo = CreateStartInfo(scriptPath, forwardedArguments, false);
    ...
    public static BackgroundLaunchResult StartBackground(string scriptPath, IReadOnlyList<string> forwardedArguments) {
        if (!File.Exists(scriptPath)) return BackgroundLaunchResult.Failure($"Script not found: {scriptPath}", 10);

        var startInfo = CreateStartInfo(scriptPath, forwardedArguments, true);
    ...
    private static ProcessStartInfo CreateStartInfo(
        ...
        var startInfo = new ProcessStartInfo { FileName = "powershell.exe", UseShellExecute = useShellExecute };
