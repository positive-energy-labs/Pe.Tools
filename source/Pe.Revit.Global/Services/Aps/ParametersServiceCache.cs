...
public static class ParametersServiceCache {
    private const string CacheFilename = "parameters-service-cache";

    public static async Task<ParametersServiceCacheData> RefreshAsync() {
        var globalState = StorageClient.Default.Global().State();
        var cache = globalState.Json<ParametersApi.Parameters>(CacheFilename);
        var cacheFilePath = ((JsonReader<ParametersApi.Parameters>)cache).FilePath;

        var tokenProvider = new CacheParametersServiceSettings();
        var accessToken = AcquireParameterServiceAccessToken();
        var parameters = await CreateParametersClient(accessToken, tokenProvider)
            .GetParameters(cache, false)
        ...
        return new ParametersServiceCacheData(
            parameters?.Results?.Count ?? 0,
            cacheFilePath,
            additionalFormatPaths
        );
    ...
    private static void WriteCsv(List<EnrichedParameterData> data, string filePath) {
        ...
        foreach (var item in jsonArray) {
            var values = properties.Select(prop => ConvertJsonValueToCsvField(item[prop]));
    ...
    private static Dictionary<string, (int start, int end)> GetSectionRanges(List<EnrichedParameterData> data) {
        const int SectionQuantityThreshold = 4;
        const int MinConsecutiveCount = 3;
        var prefixLength = 1;
        Dictionary<string, (int start, int end)> sections = [];

        while (sections.Count < SectionQuantityThreshold && prefixLength <= 10) {
            sections = IdentifySections(data, prefixLength, MinConsecutiveCount);
            prefixLength++;
        }
        ...
        for (var i = 1; i < data.Count; i++) {
            ...
            if (prefix != currentPrefix) {
                ...
                if (sectionLength >= minConsecutiveCount && !string.IsNullOrEmpty(currentPrefix))
                    sections[currentPrefix] = (sectionStart + 1, i);
...
public sealed class EnrichedParameterData {
