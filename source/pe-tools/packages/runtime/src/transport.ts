...
export interface RuntimeLocalTransportAuth {
  token: string;
  isAuthorized(request: IncomingMessage, url: URL): boolean;
}

export interface RuntimeLocalTransportAuthOptions {
  token?: string;
  headerNames?: string[];
...
export function createRuntimeLocalTransportAuth(
  options: RuntimeLocalTransportAuthOptions = {},
): RuntimeLocalTransportAuth {
  const token = options.token ?? randomBytes(24).toString("base64url");
  const headerNames = (options.headerNames ?? ["x-runtime-local-token", "x-pea-local-token"]).map(
