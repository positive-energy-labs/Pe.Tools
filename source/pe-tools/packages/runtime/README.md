...
`@pe/runtime` owns the protocol-neutral runtime seam used by Pe.Tools agent transports. It should not know whether the caller is the user-facing Pea product or the private `pe-code` development agent.
...
- `pe-code` repo-root discovery, dev-agent model defaults, repo skills, or live-loop policy;
...
- `source/pe-tools/apps/pe-code/src/runtime.ts` builds the `pe-code` runtime factory.
