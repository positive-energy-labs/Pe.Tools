...
export type TargetResolution =
  ...
  | { kind: "resolved"; mode: "pinned" | "implicit"; selector: TargetSelector; session: SessionFacts }
...
function matches(session: SessionFacts, selector: TargetSelector): boolean {
  ...
  if (selector.startsWith("sandbox:")) return session.sandboxId === selector.slice("sandbox:".length);
...
export function resolveTarget(
  ...
  if (hits.length === 0)
    return { kind: "unresolved", selector, reason: sessions.length === 0 ? "no-sessions" : "no-match" };
