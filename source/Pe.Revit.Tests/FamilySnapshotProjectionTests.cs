...
public sealed class FamilySnapshotProjectionTests {
    [Test]
    public void
        ProjectProfiles_empty_allowed_preserves_definition_only_family_and_cache_classified_shared_parameters() {
        ...
        var sharedParameterNames = new HashSet<string>(StringComparer.Ordinal) { "SharedWidth" };
        ...
        Assert.Multiple(() => {
            Assert.That(projection.EmptyAllowedProfile.AddFamilyParams.Parameters.Select(parameter => parameter.Name),
                Does.Contain("Width"));
            Assert.That(projection.EmptyAllowedProfile.FilterApsParams.IncludeNames.Equaling,
                Does.Contain("SharedWidth"));
            Assert.That(projection.EmptyAllowedProfile.AddFamilyParams.Parameters.Select(parameter => parameter.Name),
                Does.Not.Contain("SharedWidth"));
            Assert.That(projection.DenseProfile.AddFamilyParams.Parameters.Select(parameter => parameter.Name),
    ...
    public void ProjectProfiles_keeps_value_assignments_in_both_variants() {
        ...
        Assert.Multiple(() => {
            Assert.That(
                ...
                Does.Contain("Width"));
            Assert.That(
                projection.EmptyAllowedProfile.SetKnownParams.GlobalAssignments.Select(assignment =>
