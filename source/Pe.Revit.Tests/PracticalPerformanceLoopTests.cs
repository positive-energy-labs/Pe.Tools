...
[Explicit(
    "Practical Revit performance loops. Runs repeated open/action/close benchmarks against generic staged documents.")]
public sealed class PracticalPerformanceLoopTests {
    ...
    public void FF_manager_roundtrip_can_repeat_on_staged_generic_family_document() {
        var summary =
            PracticalBenchmarks.RunFamilyFoundryRoundtrip(this._dbApplication, log: TestContext.Progress.WriteLine);
    ...
    public void Loaded_families_matrix_can_repeat_on_staged_project_document() {
        var summary =
            PracticalBenchmarks.RunLoadedFamiliesMatrix(this._dbApplication, log: TestContext.Progress.WriteLine);
        Assert.That(summary.IterationCount, Is.EqualTo(PracticalBenchmarks.DefaultIterations));
    ...
    public void FF_migrator_style_queue_can_repeat_on_staged_project_document() {
        var summary =
            PracticalBenchmarks.RunFamilyFoundryMigratorQueue(this._dbApplication, log: TestContext.Progress.WriteLine);
        Assert.That(summary.IterationCount, Is.EqualTo(PracticalBenchmarks.DefaultIterations));
    ...
    public void Parameter_assignment_paths_can_repeat_on_staged_family_document() {
        var summary =
            PracticalBenchmarks.RunParameterAssignmentPaths(this._dbApplication, log: TestContext.Progress.WriteLine);
        Assert.That(summary.IterationCount, Is.EqualTo(PracticalBenchmarks.DefaultIterations));
