    ...
    try
    {
        ...
        foreach ($entry in $entries)
        {
            ...
            $recentOpens[$entry.Path] = [pscustomobject]@{
                ...
                RevitPid = if ($entry.PSObject.Properties.Name -contains 'RevitPid')
                {
                    $entry.RevitPid
                }
                else
...
function Save-RecentlyOpenedFiles
{
    ...
    foreach ($file in $ResolvedFiles)
    {
        $recentOpens[$file] = [pscustomobject]@{
            ...
            RevitPid = if ($null -ne $CurrentRevitSession)
            {
                $CurrentRevitSession.Id
            }
            else
            {
                $null
            }
            RevitStartUtc = if ($null -ne $CurrentRevitSession)
            {
                $CurrentRevitSession.StartUtc
            }
            else
            {
                $null
