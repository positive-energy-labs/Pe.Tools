...
- Start with compact context/catalog/resolve operations. Use `revit.catalog.project-browser` when user language is browser/navigation-shaped (`printed`, `design`, `reference`, `archive`, `x sheets`, `???`, folder names); use semantic catalog/matrix/detail operations for BIM facts. Escalate only with explicit filters, `budget`, and projection/view choices; malformed filters should fail or diagnose, not silently broaden.
