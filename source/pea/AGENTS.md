...
- For visible/current/printed equipment audits, resolve the view/sheet phrase once with printed/view narrowing when available, reuse those handles for the turn unless status/context changes, then use `revit.context.visible-summary` with `ViewReferences` and handles-only output or call `revit.matrix.schedule-coverage` directly with `ViewReferences`. Request compact mismatch fields (`missingHandles`, `matchedScheduleNames`) before broad samples. Avoid repeated `revit.resolve.references`, broad `revit.catalog.schedules`, and model-wide coverage unless the user asks for them.
- For per-equipment electrical alignment, `revit.detail.elements` is the exact element-to-electrical join surface. Inspect selected, visible, or explicit handles with requested parameters such as `Mark`, `Panel`, `Circuit Number`, and `Load Name` before broad panel/circuit operations.
- Treat `revit.detail.electrical-panel-schedules` as downstream row/cell detail after known panel or circuit candidates exist. It is not a substitute for exact element context.
- For wide script/audit outputs, save CSV/JSON artifacts in the workspace and return paths plus summary counts; avoid wide terminal tables except for tiny excerpts.
- When bridge calls return 423/499, time out, or appear lost, surface the operation key, `requestId`, `elapsedMs`, and any lock details, assume the single-flight Revit lane may still be busy, and check logs/wait rather than launching parallel bridge calls.
