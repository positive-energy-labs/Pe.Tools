...
export class PeaRuntimeToAgUiEvents {
  ...
  translate(event: PeaRuntimeEvent): BaseEvent[] {
    switch (event.type) {
      ...
      case "tool_updated":
        return customToolEvent("pea.tool.update", event.toolCallId, event.partialResult);
      case "tool_shell_output":
        return customToolEvent("pea.tool.shell_output", event.toolCallId, {
          output: event.output,
          stream: event.stream,
        });
      case "tool_finished":
      ...
      case "plan_updated":
        return [{ type: EventType.CUSTOM, name: "pea.plan.updated", value: event.tasks }];
      case "plan_requested":
        return [
          {
            type: EventType.CUSTOM,
            name: "pea.plan.requested",
