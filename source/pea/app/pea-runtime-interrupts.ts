...
export type PeaRuntimeInterruptReason =
  | "tool_approval_required"
  | "tool_suspended"
  | "plan_approval_required"
  | "runtime_suspended";

export interface PeaRuntimeInterrupt {
  id: string;
  reason: PeaRuntimeInterruptReason;
  message?: string;
  toolCallId?: string;
  responseSchema?: PeaJsonObject;
  expiresAt?: string;
  metadata?: PeaJsonObject;
}

export type PeaRuntimeRunOutcome =
  | { type: "success" }
  | { type: "interrupt"; interrupts: PeaRuntimeInterrupt[] };

export interface PeaRuntimeResumeDecision {
  interruptId: string;
  status: "resolved" | "cancelled";
  payload?: PeaJsonValue;
}

export class PeaRuntimeInterruptCollector {
  private readonly interruptsById = new Map<string, PeaRuntimeInterrupt>();
  private planRequestCount = 0;
  private sawSuspendedRun = false;

  observe(event: PeaRuntimeEvent): void {
    switch (event.type) {
      case "tool_started":
  ...
  private observeToolStarted(event: Extract<PeaRuntimeEvent, { type: "tool_started" }>): void {
  ...
  private addInterrupt(interrupt: PeaRuntimeInterrupt): void {
    this.interruptsById.set(interrupt.id, withoutUndefinedInterrupt(interrupt));
  }
}

export function createPeaRuntimeResumeContextEntries(
  decisions: PeaRuntimeResumeDecision[] | undefined,
): Array<{ description: string; value: string }> {
  if (!decisions || decisions.length === 0) return [];
  return [
    {
      description: "Pea runtime resume decisions",
      value: JSON.stringify(decisions.map(withoutUndefinedResumeDecision), null, 2),
    },
  ];
}

export function toPeaRuntimeResumeDecisions(
  decisions:
    | Array<{ interruptId: string; status: "resolved" | "cancelled"; payload?: unknown }>
    | undefined,
...
function toolMetadata(event: Extract<PeaRuntimeEvent, { type: "tool_started" }>): PeaJsonObject {
...
function schemaObject(value: unknown): PeaJsonObject | undefined {
  if (!isPlainRecord(value)) return undefined;
  return sanitizeObject(value);
}

function sanitizeObject(value: Record<string, unknown>): PeaJsonObject {
...
function withoutUndefinedInterrupt(value: PeaRuntimeInterrupt): PeaRuntimeInterrupt {
  return Object.fromEntries(
    Object.entries(value).filter(([, entry]) => entry !== undefined),
  ) as PeaRuntimeInterrupt;
}

function withoutUndefinedResumeDecision(value: PeaRuntimeResumeDecision): PeaRuntimeResumeDecision {
  return Object.fromEntries(
    Object.entries(value).filter(([, entry]) => entry !== undefined),
  ) as PeaRuntimeResumeDecision;
}
