...
describe("Pea runtime protocol sessions", () => {
  it("creates stable Pea-owned sessions for protocol thread ids", async () => {
    ...
    assert.deepEqual(calls, ["createRuntime:C:\\repo:ag-ui", "createThread:AG-UI thread-a"]);
  });
  ...
  it("adds session resource scope and scoped prompt resources to runtime context", async () => {
    ...
    assert.equal(sent[0]?.context?.[0]?.description, "Pea runtime resource scope");
    assert.deepEqual(JSON.parse(sent[0]?.context?.[0]?.value ?? "{}"), {
      cwd: "C:\\repo",
      additionalDirectories: ["C:\\shared"],
    });
    assert.equal(sent[0]?.context?.[1]?.description, "Pea runtime resource: spec.md");
  ...
  it("persists pending resume decisions and consumes them at the next prompt", async () => {
    const temp = mkdtempSync(path.join(os.tmpdir(), "pea-runtime-pending-resume-"));
    ...
    try {
      ...
      await secondManager.resumeSession(session.id, { cwd: "C:/repo", protocol: "acp" });
  ...
  it("returns cancelled when the active turn is aborted", async () => {
    ...
    assert.equal(await manager.sendPrompt(session.id, { content: "stop" }), "cancelled");
  });
  ...
  it("persists protocol session records and rehydrates them for metadata-only resume", async () => {
    ...
    try {
      ...
      assert.deepEqual(firstManager.listSessions({ protocol: "acp" }), [
        {
          ...
          updatedAt: firstManager.listSessions({ protocol: "acp" })[0]?.updatedAt,
          promptActive: false,
        },
      ]);
      ...
      assert.equal(secondManager.listSessions({ protocol: "acp" })[0]?.id, "test-1");
      ...
      assert.deepEqual(resumed.additionalDirectories, ["C:\\new"]);
      assert.deepEqual(secondCalls, ["createRuntime:C:\\repo:acp", "createThread:ACP dev-agent"]);
      assert.equal(
        (await secondManager.createSession({ protocol: "acp", cwd: "C:/repo" })).id,
        "test-2",
      );
      ...
      assert.deepEqual(
        thirdManager.listSessions({ protocol: "acp" }).map((record) => record.id),
        ["test-2"],
      );
    } finally {
      rmSync(temp, { recursive: true, force: true });
  ...
  it("forks persisted protocol history into a new runtime thread", async () => {
    const temp = mkdtempSync(path.join(os.tmpdir(), "pea-runtime-fork-sessions-"));
    const registryPath = path.join(temp, "sessions.json");
    const secondCalls: string[] = [];
    ...
    try {
      ...
      assert.deepEqual(secondCalls, [
        "createRuntime:C:\\repo:acp",
        "createThread:ACP dev-agent fork",
      ]);
  ...
  it("persists AG-UI external thread mappings across runtime session managers", async () => {
    const temp = mkdtempSync(path.join(os.tmpdir(), "pea-runtime-agui-sessions-"));
    const registryPath = path.join(temp, "sessions.json");
    const secondCalls: string[] = [];
      ...
      return {
        ...
        sessions: {
          async createThreadSession(options?: { title?: string }) {
            ...
            return { threadId: `thread-${threadNumber}`, resourceId: `resource-${threadNumber}` };
          },
