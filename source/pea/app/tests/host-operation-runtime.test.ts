...
describe("host operation discovery", () => {
  it("renders a compressed schedule and parameter capability map for broad discovery", () => {
    const result = capabilityMapResult(searchHostOperations({
  ...
  it("can return normalized capability-map rows as json", () => {
    ...
    assert(catalogSection.rows.some((row) => row.key === "revit.catalog.parameter-evidence"));
    assert(catalogSection.rows.some((row) => row.input.includes("candidateParameters")));
  });
  ...
  it("keeps TOON as an optional preview rendering only", () => {
    ...
    assert.match(result.rendered ?? "", /rows\[\d+\]\{key,area,description,safety,input,output,relations,terms\}:/);
    assert(result.matchedOperationKeys.includes("revit.matrix.schedule-coverage"));
  });

  it("preserves full ranked operation metadata for exact execution planning", () => {
    const results = fullResults(searchHostOperations({
      projection: "matches",
      query: "revit matrix schedule coverage",
      verbosity: "full",
      limit: 8,
    }));
    const scheduleCoverage = results.find((result) => result.key === "revit.matrix.schedule-coverage");
  ...
  it("shows DTO-aligned safe defaults for query-wrapper detail operations", () => {
    ...
    const scheduleProfiles = results.find((result) => result.key === "revit.matrix.schedule-profiles");
