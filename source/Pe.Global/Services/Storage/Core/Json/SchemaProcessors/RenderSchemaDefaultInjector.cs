...
/// <summary>
///     Injects schema defaults using the default-instance strategy from storage serialization.
/// </summary>
internal static class RenderSchemaDefaultInjector {
    public static void ApplyDefaults(JObject schemaRoot, Type rootSettingsType) {
        var defaultInstance = DefaultInstanceFactory.TryCreateDefaultInstance(rootSettingsType);
        ApplyDefaultsRecursive(schemaRoot, rootSettingsType, defaultInstance, isFragmentRoot: false);
    }

    public static void ApplyFragmentDefaults(JObject fragmentSchemaRoot, Type itemType) =>
        // Fragment schema root is a wrapper object with an Items array of the settings item type.
        ApplyDefaultsRecursive(fragmentSchemaRoot, itemType, null, isFragmentRoot: true);
    ...
    private static void ApplyDefaultsRecursive(
        ...
        if (schemaObject.TryGetValue("properties", out var propertiesToken) &&
            ...
            foreach (var schemaProperty in propertiesObject.Properties()) {
                ...
                if (schemaProperty.Value is not JObject childSchema)
                    continue;
                ...
                ApplyDefaultsRecursive(childSchema, propertyType, propertyDefault, isFragmentRoot: false);
            }
        }
        ...
        ApplyDefaultsRecursive(itemSchema, itemType, itemDefaultInstance, isFragmentRoot: false);
    }
    ...
    private static PropertyInfo? ResolvePropertyBySchemaName(Type type, string schemaPropertyName) {
        ...
        var byJsonName = props.FirstOrDefault(p =>
            string.Equals(p.GetCustomAttribute<JsonPropertyAttribute>()?.PropertyName, schemaPropertyName,
