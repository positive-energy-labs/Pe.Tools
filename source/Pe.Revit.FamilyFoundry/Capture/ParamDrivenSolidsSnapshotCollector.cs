...
public partial class ParamDrivenSolidsSnapshotCollector : IFamilySnapshotCollector {
    ...
    private static RefPlaneSnapshot? RemoveInternalAuthoredConstraints(
        ...
        return new RefPlaneSnapshot {
            ...
            MirrorConstraintSnapshots = refPlaneSnapshot.MirrorConstraintSnapshots
                .Where(spec => !internalMirrorKeys.Contains(BuildMirrorKey(spec)))
                .ToList(),
            OffsetConstraintSnapshots = refPlaneSnapshot.OffsetConstraintSnapshots
    ...
    private static void PromoteResidualAuthoredPlanes(
        ...
        foreach (var spec in refPlaneSnapshot.OffsetConstraintSnapshots) {
    ...
    private static string BuildResidualAuthoredPlaneName(OffsetConstraintSnapshot spec, ISet<string> usedNames) {
    ...
    private static bool TryBuildInlineHeightPlane(
        ...
        height = null!;
        var offsetMatch = TryMatchOffsetConstraintSnapshot(pair, refPlanesAndDims?.OffsetConstraintSnapshots ?? []);
    ...
    private static bool TryBuildRectangleSpec(
        Extrusion extrusion,
        Document doc,
        out ConstrainedRectangleExtrusionSnapshot spec
    ) {
        spec = null!;
        ...
        if (!TryGetProfilePlanePairs(doc, extrusion, out var pairA, out var pairB)) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Rectangle extrusion {ExtrusionId} could not resolve profile plane pairs.",
                extrusion.Id.Value());
            return false;
        }
        ...
        spec = new ConstrainedRectangleExtrusionSnapshot {
    ...
    private static bool TryBuildCircleSpec(
        ...
        Document doc,
        RefPlaneSnapshot? refPlanesAndDims,
        out ConstrainedCircleExtrusionSnapshot spec
    ) {
        spec = null!;
        ...
        if (!TryGetCircleCenterPlanes(doc, extrusion, out var centerLeftRightPlane, out var centerFrontBackPlane)) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} could not resolve center planes.",
                extrusion.Id.Value());
            return false;
        }
        ...
        if (!TryGetCircleDiameterParameter(extrusion, doc, out var diameterParameter)) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} could not resolve a labeled diameter parameter.",
                extrusion.Id.Value());
            return false;
        }
        ...
        spec = new ConstrainedCircleExtrusionSnapshot {
            Name = GetExtrusionName(extrusion),
            IsSolid = extrusion.IsSolid,
            StartOffset = extrusion.StartOffset,
            EndOffset = extrusion.EndOffset,
    ...
    private static bool TryGetCirclePlanePairsFromRefPlaneSnapshot(
        ...
        if (refPlanesAndDims == null) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} had no RefPlaneSnapshot available for fallback.",
                extrusion.Id.Value());
            return false;
        }
        ...
        if (sketchPlane == null) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} had no sketch plane for fallback.",
                extrusion.Id.Value());
            return false;
        }
        ...
        if (arcs.Count == 0) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} had no arcs in sketch profile for fallback.",
                extrusion.Id.Value());
            return false;
        }
        ...
        Log.Debug(
            "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} fallback evaluating {MirrorConstraintSnapshotCount} mirror specs at center {Center} radius {Radius}.",
            extrusion.Id.Value(),
            refPlanesAndDims.MirrorConstraintSnapshots.Count,
            center,
            radius);
        var candidates = refPlanesAndDims.MirrorConstraintSnapshots
        ...
        Log.Debug(
            "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} fallback produced {CandidateCount} candidate plane pairs.",
            extrusion.Id.Value(),
            candidates.Count);

        if (candidates.Count < 2) {
            var topologicalCandidates = refPlanesAndDims.MirrorConstraintSnapshots
            ...
            if (exactParameterMatch != null) {
                Log.Debug(
                    "[ParamDrivenSolidsSnapshotCollector] Circle extrusion {ExtrusionId} fallback recovered plane pairs topologically from parameter {Parameter}.",
                    extrusion.Id.Value(),
    ...
    private static CircleMirrorPairCandidate? TryBuildCircleMirrorPairCandidate(
        ...
        if (centerPlane == null) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle fallback rejected mirror spec {MirrorConstraintSnapshotName}: center anchor {CenterAnchor} missing.",
                spec.Name,
                spec.CenterAnchor);
            return null;
        }
        ...
        if (Math.Abs(centerNormal.DotProduct(sketchNormal)) > DotOrthoTolerance) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle fallback rejected mirror spec {MirrorConstraintSnapshotName}: center normal {CenterNormal} not orthogonal to sketch normal {SketchNormal}.",
                spec.Name,
                centerNormal,
                sketchNormal);
            return null;
        ...
        if (plane1 == null || plane2 == null) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle fallback rejected mirror spec {MirrorConstraintSnapshotName}: side planes {Plane1Name} / {Plane2Name} missing.",
                spec.Name,
        ...
        if (Math.Abs(distance1 - radius) > 1e-3 || Math.Abs(distance2 - radius) > 1e-3) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Circle fallback rejected mirror spec {MirrorConstraintSnapshotName}: distances {Distance1}, {Distance2} did not match radius {Radius}.",
                spec.Name,
                distance1,
                distance2,
                radius);
    ...
    private static bool TryGetProfilePlanePairs(
        ...
        if (profilePlanes.Count < 4) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Extrusion {ExtrusionId} found only {PlaneCount} profile planes: {PlaneNames}",
                extrusion.Id.Value(),
                profilePlanes.Count,
        ...
        if (grouped.Count < 2) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Extrusion {ExtrusionId} profile planes could not form two parallel groups. Planes: {PlaneInfo}",
                extrusion.Id.Value(),
    ...
    private static List<ReferencePlane> GetProfileReferencePlanes(Document doc, Extrusion extrusion) {
        ...
        if (matchedPlanes.Count > 0) {
            Log.Debug(
                "[ParamDrivenSolidsSnapshotCollector] Extrusion {ExtrusionId} matched {PlaneCount} profile planes after fallback: {PlaneNames}",
                extrusion.Id.Value(),
                matchedPlanes.Count,
    ...
    private static string BuildOffsetKey(OffsetConstraintSnapshot spec) =>
