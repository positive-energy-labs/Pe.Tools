...
Make authored family-processing workflows predictable, debuggable, and reusable across authoring, capture, projection,
apply, and targeted automation.

Family Foundry should own semantic authoring intent, compilation, replay policy, and proof artifacts while leaning on
deeper reusable family/document modules for the low-level Revit mechanics.
...
- Push reusable family/document mechanics down into `Pe.Revit.Extensions` and `Pe.Revit.Global` instead of letting
  `Pe.App` commands or feature orchestration own them.
- Treat `Document`, `FamilyDocument`, `FamilyManager`, and related extension surfaces as the preferred entrypoints for
  reusable family-side collect/capture/apply flows.
- Keep Family Foundry-owned snapshots, projections, and apply policy local until they prove broader than Family Foundry
  itself.
...
- Consume generalized document/family mechanics from sibling packages when the concept is broader than Family Foundry
  itself.
- Keep feature-owned semantic layers such as authored profiles and compilers local even when the low-level mechanics are
  extracted.
...
- Do not let Family Foundry become the accidental home of reusable document collectors or generic family mutation
  helpers merely because it needed them first.
