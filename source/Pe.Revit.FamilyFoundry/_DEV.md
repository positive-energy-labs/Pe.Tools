...
Family Foundry is an authored-workflow engine for family mutation and proof. Profiles describe intent, queues and operations execute that intent, snapshots prove what existed before or after, compilers bridge compact authored models into runnable plans, and pipeline-owned artifacts are the main transparency surface for humans, agents, and tests.
...
- `artifact manifest` means the per-family output map recorded on `FamilyProcessingContext.Artifacts`, not a separate persistence model.
...
- `AuthoredParamDrivenSolidsSettings` is authored profile content, but its compiler output becomes execution settings like `ParamDrivenPlanesAndDimsPlan` and `ParamDrivenExtrusionsPlan`. That is a real authored -> resolved-execution transition and should be described that way.
...
5. `ProcessingResultBuilder` writes run/family artifacts such as `family-report.json`, snapshot diffs, projections, and operation plans.
...
5. Use projected snapshot profile artifacts and authored-plan artifacts when validating capture/projection/compiler correctness instead of relying on logs alone.
...
- Decide how much of the current reflective snapshot-profile projection in `ProcessingResultBuilder` should become a deeper shared contract without reintroducing awkward package boundaries.
