...
- `Installer.PeDevCliComponent.cs` - PATH-visible `pe-dev` operator CLI.
...
| Term | Meaning |
| --- | --- |
| **installer product slice** | User-facing install concern such as `desktop-runtime`, `pea-cli-bootstrap`, or `pe-dev-cli-bootstrap`. A slice may contain multiple MSI components. |
