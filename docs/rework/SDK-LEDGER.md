...
| id | status | item |
|---|---|---|
| T1 | fixed@in-tree | Drizzle: fixed via SEA require shim (vite.config.ts plugin — plain `neverBundle` dies with ERR_UNKNOWN_BUILTIN_MODULE; static ESM externals cannot load inside the SEA) + JS sidecar staging. GATE MET (with T9): alt-port repro shows `/pe/info` 200 + `agentRuntime.available:true` installed. Awaiting orchestrator commit. |
