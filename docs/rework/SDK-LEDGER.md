...
| T7 | fixed@eded9a6 (partial) | BUILD.md lane/shim/PATH sections rewritten to SDK verbs + PePayloadContext truth. REMAINING: the `bin\pea` layout section (~:185-208) awaits T5's revived payload shape (old @opentui native line is also stale — custom TUI is dead per D5 clarification). |
