...
- ~~**host → service-file convention (Phase 4)**~~ DONE in the squash: host writes state/service/host.json on bind (actual port + token); addin installed lane uses EnsureRunning("host").
...
- **Coordinated effect beta.94 bump** (post-squash, codex compile-smash candidate): host stayed on beta.92 — `effect/Schema`'s compile-time AST is NOT compatible beta.92↔94 (Filter/Check, InvalidType, Option shapes), so host + host-contracts + web + mcps must bump together. No beta.94-only API is needed; runtime HTTP surfaces verified identical. See SQUASH-DESIGN.md.
- **Delete `runRuntimeAgentControllerWeb` + server bits in packages/runtime** once peco's `web` subcommand migrates off it (peco still consumes it incl. the `resolveServingTarget` createMastraCode wrap branch; dead for pea/host).
- **Port-0 fallback for the host** (A10 "actual wins" completion): host still requires preferred 5180 free; add a fallback to port 0 so a squatted port degrades to service-file discovery instead of boot failure.
- **thread-lock.ts as an Effect service** (~half day): acquireRelease + Schedule replaces the hand-rolled retry loop; exit hooks become backstop; file protocol stays byte-compatible for the TUI. Best remaining effectification candidate in packages/runtime — the rest is pure data-shaping, leave it.
- **`/host/update` self-shutdown**: route stages and returns without self-shutdown today (host self-update as VersionedApp is the real story); wire it through the shutdown latch when host update joins the release flow. Also: CmdFFMigrator/CmdFFDesiredMigrator error text still points users at "external Pe Tools route / PE_TOOLS_FRONTEND_BASE_URL" — now misleading UX, not just stale wording.
- **Debrief harvest (2026-07-08, post-squash)**: (a) host-ownership takeover/waitForPortRelease still probes the hardcoded-port base URL — at odds with the actual-port-via-service-file doctrine; (b) the hard-exit insurance timer in host index.ts is untested by construction; (c) C# installed-lane port propagation is env-var global plumbing (`PE_TOOLS_HOST_BASE_URL` set inside EnsureInstalledHostRunning) — works, but callers (BridgeAgent, TsHostCallClient, CmdAutoTag) should eventually read the service file; (d) EnsureRunning timeout passed as 8s (legacy `DefaultHostStartupTimeoutMs`) vs SDK default 15s — unowned decision, and the squashed host now boots Mastra; revisit before release; (e) permanent pnpm "cyclic workspace dependencies" warning from @pe/runtime ↔ @pe/mcps — tolerated, bites if vp ever builds topologically; (f) `runtime.mastra`/`storage` cross the Mastra seam as `unknown` + `as never` casts — inherited looseness; (g) vite-plus has no typecheck-only subcommand (agents drop to raw tsc to isolate breakage) — SDK/vp wish.
