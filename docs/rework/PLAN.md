...
- **`/host/update` self-shutdown**: route stages and returns without self-shutdown today (host self-update as VersionedApp is the real story); wire it through the shutdown latch when host update joins the release flow. Also: CmdFFMigrator/CmdFFDesiredMigrator messages still say "external Pe Tools route" (now the host origin) — cosmetic.
