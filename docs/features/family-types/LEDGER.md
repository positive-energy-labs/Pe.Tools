...
| 21 | done | P5 e2e proved the full loop live: refresh_snapshot + parse_spec (real LlamaParse, 26 blocks) → agent proposals w/ provenance over the mask → browser accept/stage → human push → values verified in Revit (Voltage 115V, FLA 1.2A, MCA 3.0A, MOCP 15A; ApparentPower formula recomputed to 276 VA; flagged Manufacturer correctly held) |
| 22 | done | No programmatic path into the family editor existed (EditFamily is transaction-forbidden in the scripting lane; IDC_EDIT_LOADED_FAM not postable) — added `family.editor.open` host op (754b8e1) |
| 23 | open | Browser-pane screenshot capture timed out all session (page healthy; DOM/text tools fine) — pixel-level taste pass on /family-types still owed; retry in a fresh session |
| 24 | open | peco MCP tools report stale bridge state after a TS-host restart (pe_status said disconnected while /host/status said connected) — MCP process caches; restart MCP or probe /host/status directly |
| 25 | open | capture_view with no viewId returned a project view while the family editor was active — family-editor evidence capture needs a viewId (or investigate active-UIDocument resolution) |
| 26 | open | family.editor.open leaves the scratch .rfa under %TEMP%\pe-family-editor and the edited family is NOT loaded back into the project on push — load-back-to-project op is the natural next step (pairs with ledger 6) |
