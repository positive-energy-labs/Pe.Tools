...
- Default element context payloads should not expose company-standard custom parameters.
- Request-scoped custom parameter reads belong behind `query.parameterQuery.parameterNames`.
...
- `POST /api/revit-data/electrical/circuits/catalog` stays native-first by default.
- Default circuit entries should expose proxy hints, not company-standard identity parameters.
- Request-scoped custom parameter reads belong behind `options.parameterQuery.parameterNames`.
- Nearby proxy inference belongs behind `options.includeNearbyProxyContext`.
- Panel schedule workflows should normally join body-cell `circuitId` values to circuit catalog entries instead of making the panel schedule route heuristic-heavy.
