...
| Pea operator workbench    | `source/pe-tools/apps/pea/`                                                                                                                                            | TypeScript/Node Revit/operator CLI, agent TUI, and workbench web adapter. Bare `pea` launches the operator workbench in the source-linked dev lane; source-linked `pea web` launches the dev web supervisor. | Pea should choose among host operations, scripts, artifacts, and diagnostics; it should not encode repo-source, build, or RRD assumptions.                                                                                          |
| Agent workbench UI        | `source/pe-tools/packages/agent-contracts`, `source/pe-tools/packages/runtime/src/workbench-web.ts`, `source/pe-tools/apps/website/`, `source/pe-tools/tools/dev-web/` | Shared TypeScript workbench projection and local web transport over Mastra sessions. The website renders `@pe/agent-contracts`; Pea adapts its sessions through the shared runtime workbench server.               | UI shells should depend on shared contracts and runtime workbench projection, not on Pea internals. Future web/TUI shells should reuse the same lifecycle, thread, send, approve, model, mode, access, and hydration behavior. |
| Dev CLI and proof tooling | `source/Pe.Dev.Cli/`, SDK `pe-revit`, `BUILD.md`                                                                                                                       | SDK `pe-revit` owns Revit proof lanes and dev/live-loop mechanics; repo-local `pe-dev` owns Pea source linking, web dev supervision, and automation commands. There is no separate dev agent surface (`peco` was deleted); `pea --prompt` is the headless black-box Pea probe.                                                                                                | Developer/operator tooling only; not a product semantic layer.                                                                                                                                                                      |
