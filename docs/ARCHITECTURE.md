...
| Pea operator workbench    | `source/pe-tools/apps/pea/`                                                                   | TypeScript/Node Revit/operator CLI and agent TUI. Bare `pea` launches the operator workbench in the source-linked dev lane; command subgroups remain available for scripts and host operations. | Pea should choose among host operations, scripts, artifacts, and diagnostics; it should not encode repo-source, build, or RRD assumptions. |
| Peco repo workbench       | `source/pe-tools/apps/pe-code/`                                                               | TypeScript/Node repo/dev CLI and agent TUI exposed as `peco`; includes repo proof/live-loop tools plus Pea black-box feedback tools.                 | Peco owns repo-source posture, build topology, RRD/Rider assumptions, and skills; keep that posture out of Pea.                            |
| Agent workbench UI        | `source/pe-tools/packages/agent-*`, `source/pe-tools/packages/acp-client`, `source/pe-tools/packages/workbench-core`, `source/pe-tools/packages/tui` | Shared TypeScript UI boundary for ACP-compatible agents, retained chat/tool/plan projection, and terminal/web workbench shells.                     | UI shells should depend on ACP/client/projection/workbench contracts, not on Pea/Peco runtime internals; Pea and Peco adapt their agents through ACP. |
